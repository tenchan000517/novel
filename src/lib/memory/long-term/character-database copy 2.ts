...
export class CharacterDatabase {
    ...
    private storage: any; // StorageProviderの動的な型
    ...
    async initialize(): Promise<void> {
        ...
        try {
            ...
            logger.error('Failed to acquire storage provider', { error });
    ...
    private async loadMasterRecordsWithFallback(): Promise<void> {
        ...
        for (const masterPath of masterPaths) {
            try {
                if (await this.storage.fileExists(masterPath)) {
                    const content = await this.storage.readFile(masterPath);
    ...
    // ============================================================================
    // 2箇所重複解決システム（🆕 競合解決機能強化）
    // ============================================================================

    /**
     * 🆕 リトライ機能付きキャラクター統合処理（2箇所重複解決）
     */
    ...
    private async loadCharactersFromStorageWithFallback(): Promise<Character[]> {
        ...
        for (const basePath of characterPaths) {
            try {
                // ディレクトリの存在確認
                if (await this.storage.directoryExists(basePath)) {
                    const files = await this.storage.listFiles(basePath);
    ...
    async saveMasterRecords(): Promise<void> {
        try {
            ...
            await this.storage.writeFile(masterPath, JSON.stringify(records, null, 2));

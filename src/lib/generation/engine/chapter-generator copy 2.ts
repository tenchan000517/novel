...
export class ChapterGenerator {
    ...
    /**
     * コンストラクタ（完成版 - LearningJourneySystem遅延初期化修正版）
     */
    constructor(
        ...
        // === 🔥 修正: LearningJourneySystemの遅延初期化 ===
        // コンストラクタでは初期化せず、undefined に設定
        // initialize()メソッドでmemoryManager初期化後に作成する
        this.learningJourneySystem = undefined;
        logger.info('LearningJourneySystem initialization deferred until memoryManager is ready');

        logger.info('ChapterGenerator initialized with analysisモジュール integration (修正版 - LJS遅延初期化)');
    }
    ...
    /**
    * 内部初期化実装（修正版 - LearningJourneySystem遅延初期化対応）
    */
    private async _initialize(): Promise<void> {
        try {
            ...
            // === 🔥 修正: LearningJourneySystemの初期化（memoryManager初期化後） ===
            try {
                logger.info('Initializing LearningJourneySystem after memoryManager is ready');

                // memoryManagerが初期化済みの状態でLearningJourneySystemを作成
                this.learningJourneySystem = new LearningJourneySystem(
                    this.geminiClient,
                    memoryManager, // 初期化済みのmemoryManagerを渡す
                    characterManager
                );

                logger.info('LearningJourneySystem instance created, starting initialization');
                ...
                logger.info('✅ LearningJourneySystem initialized successfully after memoryManager');
            } catch (learningError) {
                logger.error('❌ Failed to initialize LearningJourneySystem after memoryManager', {
                ...
                // LearningJourneySystemの初期化失敗は致命的ではない
                this.learningJourneySystem = undefined;
            }
            ...
            logger.info('ChapterGenerator initialization completed (修正版)', {
                ...
                learningJourneySystemInitialized: this.learningJourneySystem?.isInitialized() || false
            });
        } catch (error) {
    ...
    async generate(
        ...
        try {
            ...
            // 「魂のこもった学びの物語」システムによる章内容の処理
            // let learningJourneyAnalysis = null;
            // if (this.learningJourneySystem && this.learningJourneySystem.isInitialized()) {
            //     try {
            //         await this.learningJourneySystem.processChapterContent(
            //             chapterNumber,
            //             content,
            //             metadata.title || `第${chapterNumber}章`
            //         );

            //         const mainConcept = context.learningJourney?.mainConcept || "ISSUE DRIVEN";
            //         const emotionAnalysis = await this.learningJourneySystem.emotion.analyzeChapterEmotion(content);
            //         const syncMetrics = await this.learningJourneySystem.emotion.analyzeSynchronization(
            //             content,
            //             mainConcept,
            //             context.learningJourney?.learningStage as LearningStage || LearningStage.EXPLORATION
            //         );

            //         learningJourneyAnalysis = { emotionAnalysis, syncMetrics };
            //     } catch (learningError) {
            //         logger.warn(`Failed to process chapter ${chapterNumber} with learning journey system`, {
            //             error: learningError instanceof Error ? learningError.message : String(learningError)
            //         });
            //     }
            // }
            ...
            if (this.contentAnalysisManager) {
                try {
                    const processingResult = await withTimeout(
                        ...
                        90000, // 🔥 修正: Post-generationに90秒制限

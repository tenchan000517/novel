/**
 * @fileoverview 完全統合記憶階層対応キャラクター検出サービス（完成版）
 * @description
 * 新しい記憶階層システム（MemoryManager）と完全統合されたキャラクター検出クラス。
 * repositoriesは使用せず、統合記憶システムのみでキャラクター検出・抽出・学習を実行。
 * AccessOptimizer統合による最適化検索、学習システム統合、品質保証機能を完全実装。
 */
...
export class DetectionService implements IDetectionService {
  ...
  constructor(private memoryManager: MemoryManager) {
    this.logger.info('DetectionService initialized with complete MemoryManager integration');
  ...
  async initialize(): Promise<void> {
    if (this.initialized) {
      this.logger.info('DetectionService already initialized');
      return;
    }
    ...
    try {
      ...
      // 検出システム固有の初期化
      await this.initializeDetectionSpecificSystems();

      // 既存の学習データの復元
      await this.restoreLearningData();

      this.initialized = true;
      this.logger.info('DetectionService complete initialization completed');

    } catch (error) {
      this.logger.error('Failed to initialize DetectionService', { error });
      throw error;
    }
  }
   ...
   * @private
   */
  private async initializeDetectionSpecificSystems(): Promise<void> {
    // 統計情報の初期化
    this.detectionStats = {
      totalDetections: 0,
      successfulDetections: 0,
    ...
    this.logger.debug('Detection-specific systems initialized');
  }
   ...
   * @private
   */
  private async restoreLearningData(): Promise<void> {
    try {
      ...
      this.logger.warn('Failed to restore learning data', { error });
    }
  }
  ...
  async detectInteractions(content: string): Promise<DetailedInteraction[]> {
    ...
    try {
      ...
      // 🔄 インタラクション結果を記憶階層システムに保存
      await this.saveInteractionAnalysisToMemorySystem(content, interactions);
  ...
  async learnFromDetectionFeedback(
    ...
    try {
      ...
      // 🔄 学習データを記憶階層システムに保存
      const saveResult = await this.saveLearningDataToMemorySystem(learningData);
  ...
  private async getAllCharactersFromMemorySystem(): Promise<Character[]> {
    try {
      ...
      this.logger.warn('No characters found in memory system');
      return [];

    } catch (error) {
      this.logger.error('Failed to get characters from memory system', { error });
      return [];
    }
  }
   ...
   * @private
   */
  private async getDetectionHistoryFromMemorySystem(
    contentHash: string
    ...
    try {
      ...
      this.logger.warn('Failed to get detection history from memory system', { error });
      return null;
    }
  }
  ...
  private async saveDetectionResultToMemorySystem(
    ...
    try {
      ...
      if (saveResult.success) {
        ...
        this.logger.warn('Detection result saving partially failed', {
          contentHash,
      ...
      this.logger.error('Failed to save detection result to memory system', {
        contentHash,
        error: error instanceof Error ? error.message : String(error)
      });
    }
  ...
  private async getDialogPatternsFromMemorySystem(characterId: string): Promise<DialogPattern[]> {
    try {
      ...
      this.logger.warn('Failed to get dialog patterns from memory system', {
        characterId,
        error: error instanceof Error ? error.message : String(error)
      });
      return this.getDefaultDialogPatterns(characterId);
   ...
   * @private
   */
  private async saveLearningDataToMemorySystem(learningData: any): Promise<{ success: boolean }> {
    try {
      ...
      this.logger.error('Failed to save learning data to memory system', {
        error: error instanceof Error ? error.message : String(error)
      });
  ...
  private async saveInteractionAnalysisToMemorySystem(
    content: string,

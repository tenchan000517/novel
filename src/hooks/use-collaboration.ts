...
export const useCollaboration = (editorId: string) => {
  ...
  const startCollaboration = useCallback(async (
    ...
    try {
      ...
      logger.info(`Starting collaboration for document ${targetDocumentId}`);
      ...
      logger.info(`Collaboration started for document ${targetDocumentId}`);
      return true;
    } catch (err) {
      setError((err as Error).message);
      logger.error(`Error starting collaboration: ${(err as Error).message}`);
      return false;
    } finally {
      setIsLoading(false);
    }
  ...
  const endCollaboration = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      setError((err as Error).message);
      logger.error(`Error ending collaboration: ${(err as Error).message}`);
      return false;
    } finally {
      setIsLoading(false);
    }
  ...
  const applyEdit = useCallback(async (
    ...
    try {
      ...
      logger.debug(`Applying edit to document ${documentId}`);
      ...
      setError((err as Error).message);
      logger.error(`Error applying edit: ${(err as Error).message}`);
  ...
  const applyEdits = useCallback(async (
    ...
    try {
      ...
      logger.debug(`Applying ${edits.length} edits to document ${documentId}`);
      ...
      setError((err as Error).message);
      logger.error(`Error applying edits: ${(err as Error).message}`);
  ...
  const saveDocument = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      setError((err as Error).message);
      logger.error(`Error saving document: ${(err as Error).message}`);
      return false;
    } finally {
      setIsLoading(false);
    }
  ...
  const resolveConflicts = useCallback(async (
    ...
    try {
      ...
      logger.info(`Resolving conflict with ${strategyIndices.length} strategies`);
      ...
      setError((err as Error).message);
      logger.error(`Error resolving conflicts: ${(err as Error).message}`);
  ...
  const updateCursorPosition = useCallback(async (
    ...
    try {
      logger.debug(`Updating cursor position to ${position}`);
      ...
      logger.error(`Error updating cursor position: ${(err as Error).message}`);
      return false;
    }
  ...
  const revertToVersion = useCallback(async (
    ...
    try {
      ...
      setError((err as Error).message);
      logger.error(`Error reverting to version: ${(err as Error).message}`);
      return false;
    } finally {
      setIsLoading(false);
    }
  ...
  const fetchVersionInfo = useCallback(async () => {
    ...
    try {
      logger.debug(`Fetching version info for document ${documentId}`);
      
      // バージョン情報取得リクエスト
      const response = await fetch(`/api/editor/collaboration?documentId=${documentId}&versions=true`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
        }
      ...
      logger.error(`Error fetching version info: ${(err as Error).message}`);
    }
  }, [sessionId, documentId]);
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 404) {
          // セッションが見つからない場合は切断
          logger.warn(`Session ${sessionId} not found, disconnecting`);
          setSessionId(null);
          setDocumentId(null);
          setDocument(null);
          setIsConnected(false);
        ...
        throw new Error(errorData.error || 'Failed to update session info');
      }
      ...
      logger.error(`Error updating session info: ${(err as Error).message}`);
    }
  }, [sessionId, document]);
  ...
  const startPolling = useCallback(() => {
    ...
    logger.debug('Started collaboration polling');
  }, [updateSessionInfo]);
  ...
  const stopPolling = useCallback(() => {
    if (pollingInterval.current) {
      ...
      logger.debug('Stopped collaboration polling');
    }
  }, []);
